...
AGENT_TRAIN_FORMAT = {"cmid": [('cmid_discriminator_loss', 'CMIDD', 'float'), ('cmid_adversarial_loss', 'CMIDA', 'float')],
                      "ted": [('ted_loss', 'TEDLOSS', 'float')],
                      "curl": [('aux_loss', 'AUXLOSS', 'float')]}
...
class Logger(object):
    def __init__(self,
        ...
        if agent == "svea_cmid":
            train_format = COMMON_TRAIN_FORMAT + AGENT_TRAIN_FORMAT["cmid"]
        elif agent == "svea_ted":
            train_format = COMMON_TRAIN_FORMAT + AGENT_TRAIN_FORMAT["ted"]
        elif agent == "curl":
            train_format = COMMON_TRAIN_FORMAT + AGENT_TRAIN_FORMAT["curl"]
        else:
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        if not self._should_log(step, log_frequency):
            return
        assert key.startswith('train') or key.startswith('eval') or key.startswith('state') or key.startswith('eval_scenarios')
